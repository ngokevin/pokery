...
function _createHand(cards, withStrength) {
  ...
  return withStrength ? (0, _holdem.getHandStrength)(hand) : hand;
}
...
describe('compareHands', function () {
  ...
  it('compares high card vs. same high card', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ac', 'Td', '7h', '5s', '3c'], true), _createHand(['As', 'Th', '7d', '5c', '3s'], true)), 0);
  });

  it('compares high card vs. worse high card', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ac', 'Td', '7h', '5s', '3c'], true), _createHand(['As', '9h', '7d', '5c', '3s'], true)), 1);
  });

  it('compares pair vs. pair with better kickers', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ac', 'Ad', '7h', '5s', '2c'], true), _createHand(['As', 'Ah', '7d', '5c', '3s'], true)), -1);
  });

  it('compares pair vs. same pair', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Kc', 'Kd', '7h', '5s', '2c'], true), _createHand(['Ks', 'Kh', '7d', '5c', '2s'], true)), 0);
  });

  it('compares pair vs. worse pair', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ac', 'Ad', '7h', '5s', '2c'], true), _createHand(['Js', 'Jh', '7d', '5c', '3s'], true)), 1);
  });

  it('compares two pair vs. better two pair', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Tc', 'Td', '5h', '5s', 'Ac'], true), _createHand(['Js', 'Jh', '3d', '3c', '2s'], true)), -1);
  });

  it('compares two pair vs. same two pair', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['4c', '4d', '3h', '3s', 'Ac'], true), _createHand(['4s', '4h', '3d', '3c', 'As'], true)), 0);
  });

  it('compares two pair vs. two pair with worse kicker', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Tc', 'Td', '5h', '5s', 'Ac'], true), _createHand(['Ts', 'Th', '5d', '5c', 'Ks'], true)), 1);
  });

  it('compares trips vs. better trips', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Tc', 'Td', 'Th', 'As', 'Kc'], true), _createHand(['Js', 'Jh', 'Jd', '3c', '2s'], true)), -1);
  });

  it('compares trips vs. worse trips', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['3c', '3d', '3h', '4s', '5c'], true), _createHand(['2s', '2h', '2d', 'Qc', 'Js'], true)), 1);
  });

  it('compares straight vs. better straight', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['5c', '6d', '7h', '8s', '9c'], true), _createHand(['6s', '7h', '8d', '9c', 'Ts'], true)), -1);
  });

  it('compares straight vs. same straight', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['6c', '7d', '8h', '9s', 'Tc'], true), _createHand(['6c', '7d', '8h', '9h', 'Td'], true)), 0);
  });

  it('compares straight vs. worse straight', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ts', 'Jh', 'Qd', 'Kc', 'As'], true), _createHand(['9c', 'Td', 'Jh', 'Qs', 'Kc'], true)), 1);
  });

  it('compares flush vs. better flush', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ac', 'Tc', '7c', '5c', '2c'], true), _createHand(['Ac', 'Tc', '7c', '5c', '3c'], true)), -1);
  });

  it('compares flush vs. same flush', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ah', 'Th', '7h', '5h', '3h'], true), _createHand(['Ah', 'Th', '7h', '5h', '3h'], true)), 0);
  });

  it('compares flush vs. worse flush', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['As', 'Ts', '7s', '5s', '3s'], true), _createHand(['As', '9s', '7s', '5s', '3s'], true)), 1);
  });

  it('compares full house vs. better full house', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ac', 'Ad', 'Ah', '2s', '2c'], true), _createHand(['Ac', 'Ad', 'Ah', '7c', '7s'], true)), -1);
  });

  it('compares full house vs. same full house', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ac', 'Ad', 'Ah', '2s', '2c'], true), _createHand(['Ac', 'Ad', 'Ah', '2c', '2s'], true)), 0);
  });

  it('compares full house vs. worse full house', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ac', 'Ad', 'Ah', '2s', '2c'], true), _createHand(['Ks', 'Kh', 'Kd', 'Qc', 'Qs'], true)), 1);
  });

  it('compares quads vs. better quads', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Kc', 'Kd', 'Kh', 'Ks', 'As'], true), _createHand(['Ac', 'Ad', 'Ah', 'As', 'Ks'], true)), -1);
  });

  it('compares quads vs. same quads', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Kc', 'Kd', 'Kh', 'Ks', 'As'], true), _createHand(['Kc', 'Kd', 'Kh', 'Ks', 'As'], true)), 0);
  });

  it('compares quads vs. worse quads', function () {
    _chai.assert.equal((0, _holdem.compareHands)(_createHand(['Ac', 'Ad', 'Ah', 'As', 'Jc'], true), _createHand(['Ac', 'Ad', 'Ah', 'As', 'Ts'], true)), 1);
  });
});